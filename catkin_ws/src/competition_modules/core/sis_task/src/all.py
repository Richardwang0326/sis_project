...
if __name__ == "__main__":
	...
	grasp_request.pose.position.y = position[1] - 0.08
	grasp_request.pose.position.z = position[2] - 0.004
	...
	br.sendTransform((position[0] - 0.02, position[1] - 0.05, position[2] - 0.008),
                        orien,
                       rospy.Time.now(),
                        "object",
                       "base_link")
	...
	try:
	   ...
	   print("service did not process request: " + str(exc))
        ...
        s.data = False
        #rospy.wait_for_service('get_object_pose')
        per = rospy.ServiceProxy('task1_prediction', SetBool)
        bo = per(s)
